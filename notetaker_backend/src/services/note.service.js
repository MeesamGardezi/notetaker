...
const createNote = async (userId, moduleId, noteData) => {
  try {
    ...
    if (moduleData.noteCount >= tier.maxNotesPerModule) {
      throw new Error(`Maximum number of notes (${tier.maxNotesPerModule}) reached for this module in your account tier`);
    }
...
const deleteNote = async (noteId, userId) => {
  try {
    ...
    // Media files should be cleaned up separately
    // This would typically be done with a Cloud Function or background process
  } catch (error) {
    throw error;
  }
};
